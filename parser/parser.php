...
class socialwiki_parser_proxy {
    ...
    public static function parse(&$string, $type, $options = array()) {
        ...
        if(empty(self::$basepath)) {
            global $CFG;
            self::$basepath = $CFG->dirroot.'/mod/socialwiki/parser/';
        }
...
abstract class socialgeneric_parser {
    protected $string;
    ...
    private function block_callback($match) {
	   $rule = end($this->rulestack);
        ...
        if(is_array($stuff) && $rule['rule']['tag']) {
            ...
            $stuff = "\n".socialparser_utils::h($rule['rule']['tag'], $stuff[0], $stuff[1])."\n";
        }
        else {
            ...
            if(isset($rule['rule']['tag']) && is_string($rule['rule']['tag'])) {
                $stuff = "\n".socialparser_utils::h($rule['rule']['tag'], $stuff)."\n";
            }
        }
    ...
    private function tag_callback($match) {
        ...
        if(is_array($stuff)) {
            return socialparser_utils::h($rule['rule']['tag'], $stuff[0], $stuff[1]);
        }
        else {
            return $stuff;
        }
