...
require_capability('mod/socialwiki:overridelock', $context);

if (!confirm_sesskey()) {
    print_error(get_string('invalidsesskey', 'socialwiki'));
}

$wikipage = new page_socialwiki_overridelocks($wiki, $subwiki, $cm);
$wikipage->set_page($page);

if (!empty($section)) {
    $wikipage->set_section($sectioncontent, $section);
}
add_to_log($course->id, "socialwiki", "overridelocks", "view.php?pageid=".$pageid, $pageid, $cm->id);
